...
import (
	"fmt"
	"github.com/anlityli/chatait-free/chatait-frontend-server/library/console/server"
...
func (c *Console) init() (err error) {
	...
	simulationArgs := g.Config().GetStrings("frontendConf.simulationArgs")
	if g.Config().GetBool("frontendConf.debugStatus") && len(simulationArgs) > 0 {
		args := []string{"main"}
