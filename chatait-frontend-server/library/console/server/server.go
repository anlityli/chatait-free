...
import (
	"github.com/anlityli/chatait-free/chatait-frontend-server/library/serv"
	"github.com/anlityli/chatait-free/chatait-frontend-server/router/ser"
...
func InitConfig() {
	// 初始化日志存放位置，日志存在文件中
	logPath := helper.FormatDirStr(g.Config().GetString("frontendConf.logPath"))
	if err := glog.SetPath(logPath); err != nil {
		glog.Line().Fatalf("设置日志文件路径失败:%s\n", err.Error())
	}
	// 如果是debug模式的话，debug日志会保存，否则debug日志不会保存
	glog.SetDebug(g.Config().GetBool("frontendConf.debugStatus"))
}
