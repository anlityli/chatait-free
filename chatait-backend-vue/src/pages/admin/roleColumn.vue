...
import { ResponseAdminRoleColumn } from '@/utils/model/response/admin'
import { RolePermissionMainCheckoutItem } from '@/pages/admin/model/model'

const router = useRouter()
const route = useRoute()

const permissionList = ref<ResponseAdminRoleColumn[]>([])
const mainPermissionCheckoutStatus = ref<RolePermissionMainCheckoutItem[]>([])

const handleCheckoutMainAll = (checked: boolean, mainItem: ResponseAdminRoleColumn, index: number) => {
  mainPermissionCheckoutStatus.value[index].checked_value = []
  for (let i = 0; i < permissionList.value[index].columns.length; i++) {
    permissionList.value[index].columns[i].is_checked = checked
    if (checked) {
      mainPermissionCheckoutStatus.value[index].checked_value.push(permissionList.value[index].columns[i].index)
    }
  }
...
const handleCheckoutChild = (checkedItems: string[], item: ResponseAdminRoleColumn, index: number) => {
  mainPermissionCheckoutStatus.value[index].checked_value = checkedItems
  mainPermissionCheckoutStatus.value[index].checkout_all =
    mainPermissionCheckoutStatus.value[index].checked_value.length === permissionList.value[index].columns.length
  mainPermissionCheckoutStatus.value[index].indeterminate =
    mainPermissionCheckoutStatus.value[index].checked_value.length > 0 &&
    !mainPermissionCheckoutStatus.value[index].checkout_all
}
...
const handleRefreshMainCheckout = () => {
  for (let i = 0; i < permissionList.value.length; i++) {
    ...
    for (let j = 0; j < permissionList.value[i].columns.length; j++) {
      if (permissionList.value[i].columns[j].is_checked) {
        childCheckoutCount += 1
        mainPermissionCheckoutStatus.value[i].checked_value.push(permissionList.value[i].columns[j].index)
      }
    }
    mainPermissionCheckoutStatus.value[i].checkout_all = childCheckoutCount === permissionList.value[i].columns.length
...
const handleSubmit = async () => {
  const requestData = {
    id: route.query.id,
    column_permission: <Record<string, string[]>>{},
  }
  for (let i = 0; i < mainPermissionCheckoutStatus.value.length; i++) {
    requestData.column_permission[mainPermissionCheckoutStatus.value[i].key] =
      mainPermissionCheckoutStatus.value[i].checked_value
  }
  await http.post('admin/role-column-permission-edit', requestData)
  router.back()
}

onMounted(async () => {
  permissionList.value = (await http.get(`admin/role-column?id=${route.query.id}`)) as ResponseAdminRoleColumn[]
  for (let i = 0; i < permissionList.value.length; i++) {
    mainPermissionCheckoutStatus.value.push({
      key: permissionList.value[i].list_id,
      checkout_all: false,
      indeterminate: false,
      checked_value: [],
    })
