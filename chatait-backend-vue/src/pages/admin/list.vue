...
<template>
  <div>
    <common-table
      ref="tableEle"
      :request-path="`admin/list`"
      :operation-column="true"
      :primary-filter-field="'updated_at'"
    ...
    >
      ...
      <template #is_enable="slotProps">
        <t-tag v-if="slotProps.params.row.is_enable.ori_value === 1" theme="success">
          {{ slotProps.params.row.is_enable.value }}
        </t-tag>
        <t-tag v-else theme="danger">{{ slotProps.params.row.is_enable.value }}</t-tag>
      </template>
...
const handleDelete = async () => {
  ...
  if (requestData.selected.length > 0) {
    await http.post('admin/delete', requestData)
