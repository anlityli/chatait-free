...
<template>
  <div>
    <common-table
      ref="tableEle"
      :request-path="`admin/role-list`"
      :operation-column="true"
      :primary-filter-field="'role_name'"
    ...
    >
      ...
      <template #operation="slotProps">
        <t-dropdown :min-column-width="88" trigger="click">
          ...
          <t-dropdown-menu>
            <t-dropdown-item @click="handlePermission(slotProps.params.row.id.ori_value)">权限</t-dropdown-item>
            <t-dropdown-item @click="handleColumn(slotProps.params.row.id.ori_value)">列表字段权限</t-dropdown-item>
...
const handleAdd = () => {
  router.push('/admin/role-add')
}

const handleEdit = (id: string) => {
  router.push(`/admin/role-edit?id=${id}`)
}

const handlePermission = (id: string) => {
  router.push(`/admin/role-permission?id=${id}`)
}

const handleColumn = (id: string) => {
  router.push(`/admin/role-column?id=${id}`)
}
...
const handleDelete = async () => {
  ...
  if (requestData.selected.length > 0) {
    await http.post('admin/role-delete', requestData)
