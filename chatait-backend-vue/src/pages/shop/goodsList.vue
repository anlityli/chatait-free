...
<template>
  <div>
    <common-table
      ...
      drag-row
      :drag-request-path="'shop/goods-sort'"
      @on-add="handleAdd"
...
const handleAdd = () => {
  router.push('/shop/goods-add')
}

const handleEdit = (id: string) => {
  router.push(`/shop/goods-edit?id=${id}`)
}
...
const handleDelete = async () => {
  ...
  if (requestData.selected.length > 0) {
    await http.post('shop/goods-delete', requestData)
